...
for step in range(last_step + 1, args.max_step + 1):
    ...
    cum_loss /= (NS_to_open*1000 / args.batch_size)
...
example_X = min_max_scalerX.transform(np.array([np.hstack((SK05, [eta050, logt] )).ravel() for logt in np.linspace(-10, 10, 100)]))
...
example_X = min_max_scalerX.transform(np.array([np.hstack((SK055, [eta055, logt] )).ravel() for logt in np.linspace(-10, 10, 100)]))
example_prediction = net(torch.tensor(example_X))
...
example_X = min_max_scalerX.transform(np.array([np.hstack((SK0515, [eta0515, logt] )).ravel() for logt in np.linspace(-10, 10, 100)]))
example_prediction = net(torch.tensor(example_X))
