...
#     rho055 = 0.55
#     # load data
#     SK05 = np.loadtxt(path+'\\Data\\Sk_data\\example_Sk_rho_%.10f.txt' %rho05)
#     Ft05 = np.loadtxt(path+'\\Data\\rho_data\\example_rho_rho_%.10f.txt' %rho05)
#     SK055 = np.loadtxt(path+'\\Data\\Sk_data\\example_Sk_rho_%.10f.txt' %rho055)
#     Ft055 = np.loadtxt(path+'\\Data\\rho_data\\example_rho_rho_%.10f.txt' %rho055)
#     SK0515 = np.loadtxt(path+'\\Data\\Sk_data\\example_Sk_rho_%.10f.txt' %rho0515)
#     Ft0515 = np.loadtxt(path+'\\Data\\rho_data\\example_rho_rho_%.10f.txt' %rho0515)
    
#     # first figure
#     plt.figure(figsize=[5,5])
#     example_X = min_max_scalerX.transform(np.array([np.hstack((SK05, [rho05, logt, Ft05[1, 1]] )).ravel() for logt in np.linspace(-6, 10, 1000)]))
...
#     #second figure
#     plt.figure(figsize=[5,5])
#     example_X = min_max_scalerX.transform(np.array([np.hstack((SK055, [rho055, logt, Ft055[1, 1]] )).ravel() for logt in np.linspace(-6, 10, 1000)]))
#     example_prediction = net(torch.tensor(example_X))
#     plt.plot(min_max_scalerX.inverse_transform(example_X)[:,-2], min_max_scalery.inverse_transform(example_prediction.drhoch().numpy())[:,0], label="DNN FAA", lw=2)
#     # plt.plot(min_max_scalerX.inverse_transform(example_X)[:,-2], min_max_scalery.inverse_transform(example_prediction.drhoch().numpy())[:,1], label="DNN FAB", lw=2)
#     plt.plot(min_max_scalerX.inverse_transform(example_X)[:,-2], min_max_scalery.inverse_transform(example_prediction.drhoch().numpy())[:,1], label="DNN FBB", lw=2)
...
#     plt.xlabel("log10(t)")
#     plt.ylim([0, 1])
#     plt.ylabel("rho(k,t) at kD=%.2f" %Ft055[1, 1])
#     plt.title("volume fraction = %.2f"%rho055)
#     plt.savefig(path+"\\predictionDNN055.png", dpi=500, bbox_inches="tight")
#     plt.show()
    
#     # third figure
#     plt.figure(figsize=[5,5])
#     example_X = min_max_scalerX.transform(np.array([np.hstack((SK0515, [rho0515, logt, Ft0515[1, 1]] )).ravel() for logt in np.linspace(-6, 10, 1000)]))
#     example_prediction = net(torch.tensor(example_X))
#     plt.plot(min_max_scalerX.inverse_transform(example_X)[:,-2], min_max_scalery.inverse_transform(example_prediction.drhoch().numpy())[:,0], label="DNN FAA", lw=2)
#     # plt.plot(min_max_scalerX.inverse_transform(example_X)[:,-2], min_max_scalery.inverse_transform(example_prediction.drhoch().numpy())[:,1], label="DNN FAB", lw=2)
#     plt.plot(min_max_scalerX.inverse_transform(example_X)[:,-2], min_max_scalery.inverse_transform(example_prediction.drhoch().numpy())[:,1], label="DNN FBB", lw=2)
...
#     plt.legend()
#     plt.xlabel("log10(t)")
#     plt.ylabel("rho(k,t) at kD=%.2f" %Ft0515[1, 1])
#     plt.title("volume fraction = %.2f"%rho0515)
#     plt.savefig(path+"\\predictionDNN0515.png", dpi=500, bbox_inches="tight")
#     plt.show()
    
#     # plot the error as function of density
#     plt.figure(figsize=[5,5])
#     error =np.sum((net(torch.tensor(X_test_scaled)).drhoch().numpy() - y_test_scaled.drhoch().numpy())**2, axis=1)
...
rho2 = 2.0
# load data
SK1 = np.loadtxt(path+'\\Data\\Sk_data\\exampleSk_rho_%.10f.txt' %rho1)
Ft1 = np.loadtxt(path+'\\Data\\phi_data\\examplephi_rho_%.10f.txt' %rho1)
SK15 = np.loadtxt(path+'\\Data\\Sk_data\\exampleSk_rho_%.10f.txt' %rho15)
Ft15 = np.loadtxt(path+'\\Data\\phi_data\\examplephi_rho_%.10f.txt' %rho15)
SK2 = np.loadtxt(path+'\\Data\\Sk_data\\exampleSk_rho_%.10f.txt' %rho2)
Ft2 = np.loadtxt(path+'\\Data\\phi_data\\examplephi_rho_%.10f.txt' %rho2)

# first figure
plt.figure(figsize=[5,5])
example_X1 = np.array([np.hstack((SK1, [rho1, Ft1[i, 0], Ft1[i, 1]] )).ravel() for i in range(200)])
example_X15 = np.array([np.hstack((SK15, [rho15, Ft15[i, 0], Ft15[i, 1]] )).ravel() for i in range(200)])
example_X2 = np.array([np.hstack((SK2, [rho2, Ft2[i, 0], Ft2[i, 1]] )).ravel() for i in range(200)])

example_prediction1 = net(torch.tensor(example_X1))
example_prediction15 = net(torch.tensor(example_X15))
example_prediction2 = net(torch.tensor(example_X2))

t_array = np.linspace(-6, 10, 200)
plt.plot(Ft1[:, 1], example_prediction1.detach().numpy(), color="red")
plt.plot(Ft15[:, 1], example_prediction15.detach().numpy(), color="blue")
plt.plot(Ft2[:, 1], example_prediction2.detach().numpy(), color="black")

plt.plot(t_array, Ft1[:, 2], color="red", linestyle="dashed")
plt.plot(t_array, Ft15[:, 2], color="blue", linestyle="dashed")
...
plt.plot(k_array, SK1, color="red")
plt.plot(k_array, SK15, color="blue")
