...
def load_data(NS_to_open, path, N_t_perfile=1000):
    ...
    for Sfile in listSfiles[:NS_to_open]:
        ...
        y[i:i+N_t_perfile, 0] = new_out_data[:, -1] # FAA
        y[i:i+N_t_perfile, 1] = new_out_data[:, -3] # FBB
        i+= N_t_perfile
...
def plot_data(net, min_max_scalerX, min_max_scalery):
    ...
    plt.savefig(path+"\\Data\\predictionDNN050.png", dpi=500, bbox_inches="tight")
    plt.show()
    ...
    plt.savefig(path+"\\Data\\predictionDNN055.png", dpi=500, bbox_inches="tight")
    plt.show()
    ...
    plt.savefig(path+"\\Data\\predictionDNN0515.png", dpi=500, bbox_inches="tight")
    plt.show()
...
explainer = shap.Explainer(net, X_train_scaled)
shap_values = explainer(X_train_scaled[0:10,:])
