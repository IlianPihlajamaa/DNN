...
def msq_loss(output, target):
    return np.sqrt(torch.mean((target - output) ** 2).item())
...
def find_analytical_C_k(k, η):
    ...
    A = -(1 - η)**-4 *(1 + 2*η)**2
    B = (1 - η)**-4*  6*η*(1 + η/2)**2
    D = -(1 - η)**-4 * 1/2 * η*(1 + 2*η)**2
    Cₖ =  4*np.pi/k**6 * (24*D - 2*B * k**2 - (24*D - 2 * (B + 6*D) * k**2 + (A + B + D) * k**4) * np.cos(k) + k * (-24*D + (A + 2*B + 4*D) * k**2) * np.sin(k))
    return Cₖ



def find_analytical_S_k(k, η):
    """
